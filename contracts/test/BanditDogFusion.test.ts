...
describe('BanditDogFusion — gacha commit/reveal, Web2.5 bridge, soul shards', () => {
  ...
  describe('tokenizeDog — Web2.5 shop-dog bridge', () => {
    it('burns the single-dog cost and mints one Chihuahua', async () => {
      const { farm, nft, fusion, backendSigner, alice } = await loadFixture(deployFixture);
    ...
    it('charges the bulk 3x price and mints three dogs', async () => {
      const { farm, nft, fusion, backendSigner, alice } = await loadFixture(deployFixture);
      ...
      await fusion.connect(alice).tokenizeDog(3n, nonce, sig);
    ...
    it('rejects counts other than 1 or 3', async () => {
      ...
      await expect(fusion.connect(alice).tokenizeDog(2n, 3n, sig))
    ...
    it('binds the signature to the caller', async () => {
      ...
      const nonce = 6n;
      const sig = await signTokenize(backendSigner, alice.address, 1n, nonce);
      await expect(fusion.connect(bob).tokenizeDog(1n, nonce, sig))
        .to.be.revertedWithCustomError(fusion, 'InvalidSignature');
    });
  });
  ...
  describe('admin', () => {
    ...
    it('freezes commit, tokenize and redeem while paused', async () => {
      ...
      await expect(fusion.connect(alice).tokenizeDog(1n, nonce, tokenizeSig))
        .to.be.revertedWithCustomError(fusion, 'EnforcedPause');
